...
describe("upsertToolCallPart", () => {
    it("pushes a new tool-call part when id is new", () => {
        const result = upsertToolCallPart([], {
            toolCallId: "tc1",
            toolName: "Bash",
            args: { command: "ls" },
        });
    ...
    it("merges updates into an existing tool-call with the same id", () => {
        const parts: MessagePart[] = [
            {
                type: "tool-call",
                toolCallId: "tc1",
        ...
        const result = upsertToolCallPart(parts, {
            toolCallId: "tc1",
            result: { stdout: "file.txt" },
        });
        expect(result).toHaveLength(1);
        // original args preserved, result patched in
        expect(result[0]).toMatchObject({
            type: "tool-call",
            toolCallId: "tc1",
            toolName: "Bash",
    ...
    it("does not dedupe by toolName — only by toolCallId", () => {
        const parts: MessagePart[] = [
            {
                type: "tool-call",
                toolCallId: "tc1",
