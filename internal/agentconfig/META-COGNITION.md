...
当用户要求查询数据统计、分析使用量、检查成本或执行任何数据库分析时，**必须先阅读 `~/.hotplex/skills/db-stats.md`**。

> [!IMPORTANT]
> **强制前置动作**：识别到数据库分析意图后，**必须先阅读 `~/.hotplex/skills/db-stats.md` 再执行任何查询**。该手册包含：1) 精准感知当前数据库类型和连接信息的方法 2) 完整数据结构参考（5 张核心表） 3) 覆盖日常分析需求的 SQL 查询模板（会话概览、成本分析、响应时间、工具使用、时间趋势等）。跳过阅读直接查询可能导致使用错误的 SQL 方言。
