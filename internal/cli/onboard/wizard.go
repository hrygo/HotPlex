...
func buildEnvContent(adminToken string, slackCfg, feishuCfg messagingPlatformConfig, existingEnvPath string) string {
	...
	if preserved := preserveUnmanagedEnv(existingEnvPath); preserved != "" {
...
func preserveUnmanagedEnv(envPath string) string {
	data, err := os.ReadFile(envPath)
	if err != nil {
		return ""
	}
	...
	generatedComments := map[string]struct{}{
		...
		"# Generated by onboard wizard":                        {},
		"# ── Security ──":                                     {},
		"# ── Worker Commands (optional overrides) ──":         {},
		"# HOTPLEX_WORKER_CLAUDE_CODE_COMMAND=claude":          {},
		"# HOTPLEX_WORKER_OPENCODE_SERVER_COMMAND=opencode":    {},
		"# ── Slack ──":                                        {},
		"# ── Feishu ──":                                       {},
		"# ── Preserved user environment ──":                   {},
	}
	...
	for _, line := range strings.Split(strings.ReplaceAll(string(data), "\r\n", "\n"), "\n") {
		...
		if strings.HasPrefix(trimmed, "#") {
			if _, generated := generatedComments[trimmed]; !generated {
				preserved = append(preserved, line)
			}
			continue
		}

		key := envAssignmentKey(trimmed)
		if key == "" {
			preserved = append(preserved, line)
			continue
		}
...
func stepBinaryInstall(reader *bufio.Reader, opts WizardOptions) StepResult {
	...
	binPath, err := install.ResolveSourceBinary()
	if err != nil {
		return StepResult{Name: stepName, Status: "fail", Detail: err.Error()}
	}
	...
	if pathBin, lpErr := exec.LookPath("hotplex"); lpErr == nil {
		...
		if install.SameContent(binPath, pathBin) {
		...
		if err := install.CopyBinary(binPath, pathBin); err != nil {
	...
	targetDir, err := install.DefaultInstallDir()
	if err != nil {
		return StepResult{Name: stepName, Status: "fail", Detail: err.Error()}
	}
	...
	if err := install.CopyBinary(binPath, targetPath); err != nil {
		return StepResult{Name: stepName, Status: "fail", Detail: "install failed: " + err.Error()}
	}

	detail := "installed to " + targetPath
	if !install.IsInPATH(targetDir) {
		if pErr := install.AddToUserPath(targetDir); pErr != nil {
