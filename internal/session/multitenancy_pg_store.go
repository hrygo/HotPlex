...
func (s *pgStore) UpdateUserPassword(ctx context.Context, id, passwordHash string, now int64) error {
	_, err := s.db.ExecContext(ctx, s.queries["users.update_password"], passwordHash, now, id)
	return err
}
