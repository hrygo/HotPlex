...
func TestDeriveSessionKey_WorkspaceIDParticipates(t *testing.T) {
	...
	without := DeriveSessionKey("u1", worker.TypeClaudeCode, "s1", "", "/tmp/p")
	withWS := DeriveSessionKey("u1", worker.TypeClaudeCode, "s1", "ws-1", "/tmp/p")
	otherWS := DeriveSessionKey("u1", worker.TypeClaudeCode, "s1", "ws-2", "/tmp/p")

	require.NotEqual(t, without, withWS, "non-empty workspaceID must change the key")
	require.NotEqual(t, withWS, otherWS, "different workspaceID → different key")
}

// TestDeriveSessionKey_EmptyWorkspaceIDBackwardCompat: empty workspaceID reproduces
// the legacy 4-field hash (platform/cron callers, spec §7.1 backward compat).
func TestDeriveSessionKey_EmptyWorkspaceIDBackwardCompat(t *testing.T) {
	t.Parallel()

	a := DeriveSessionKey("u1", worker.TypeClaudeCode, "s1", "", "/tmp/p")
	b := DeriveSessionKey("u1", worker.TypeClaudeCode, "s1", "", "/tmp/p")
	require.Equal(t, a, b)
}
