...
func stashInvocation(env *events.Envelope, invocation worker.SkillInvocation, content string) {
...
func invocationFromMetadata(md map[string]any, content string) (worker.SkillInvocation, bool) {
	raw, _ := md[metadataSkillInvocationKey].(map[string]any)
	if raw == nil {
		return worker.SkillInvocation{}, false
	}
