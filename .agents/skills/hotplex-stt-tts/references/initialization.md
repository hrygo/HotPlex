...
本 reference 只在执行初始化或修复时读取。命令中的 `$HOTPLEX_HOME` 表示当前 HotPlex 实例根目录；未设置时通常为 `~/.hotplex`。不要把本机绝对路径、令牌或完整 `.env` 内容写入仓库或回复。
...
记录非敏感事实：HotPlex home/config 路径、OS/架构、Python 版本、磁盘余量、STT/TTS provider、模型目录和服务状态。不要打印 `APP_SECRET`、Bot token、Admin token 或完整环境文件。

如果用户只要求诊断，停在这里并转交 `hotplex-cli`/`hotplex-diagnostics`；只有用户授权初始化时才继续。

## 2. Python 运行时

STT 支持 Python 3.9+；MOSS 当前官方运行时建议 Python 3.10+。两个运行时优先共用专用 venv，避免污染系统 Python：
...
安装后必须用 HotPlex 实际解析到的 `python3` 验证，而不能只验证 venv 的绝对路径：
...
如果服务的 `PATH` 只能解析系统 Python，应把 `$HOTPLEX_HOME/venv/bin` 放到服务 PATH 前部，或创建可审计的 wrapper 来 `exec` venv Python。不要只建立指向 venv Python 的软链接：某些 Python 构建会通过调用路径判断 venv，软链接可能导致 `site-packages` 消失。
...
预下载官方 SenseVoiceSmall 模型到 ModelScope cache，避免首次语音请求才联网：
...
只有用户要求启用本地 STT 或当前配置明确缺失时，才设置 `HOTPLEX_MESSAGING_STT_PROVIDER=local` 或 `feishu+local`，以及对应的 `HOTPLEX_MESSAGING_STT_LOCAL_CMD`。现有 provider 是云端 `feishu` 时，不要擅自切换为 local。

## 4. TTS：MOSS-TTS-Nano ONNX

MOSS 官方参考实现和 ONNX 资产：
...
从官方仓库取得 sidecar 脚本，从 ModelScope 下载两个 ONNX 制品：
...
官方 Python 运行时还需要 numpy、sentencepiece、ONNX Runtime、FastAPI/Uvicorn、音频和文本规范化依赖；以官方仓库当前依赖为准安装。macOS 若 `pynini`/`WeTextProcessing` 构建失败，先安装对应 OpenFst 开发文件再重试，不要删除文本规范化依赖来掩盖失败。
...
只有用户要求启用 MOSS 或当前 provider 已是 `moss`/`edge+moss` 时，才补齐：
...
保留现有 Edge TTS 配置和音色；`edge+moss` 表示 Edge 优先、MOSS 兜底，不应擅自改成纯 MOSS。
...
服务未安装时，按用户授权执行 `hotplex service install --level user`，随后用 `hotplex service status --level user --json` 验证；不要用手工 `stop && sleep && start` 替代原子 restart。
...
单独核对 `stt.runtime`、`tts.runtime`、`agent.directory_structure`、`agent.global_files` 和消息渠道状态。`doctor` 的其他警告（例如 worker 权限模式）要原样报告，不要为了让总数变绿而越权修改。
