...
─────────────────────────────────────────────────────────────
AEP v1 协议              22    7    1    30     27    90%
WebSocket Gateway         5    3    0     8      8   100%
Session 管理              6    2    0     8      8   100%
Worker 抽象与进程管理     5    6    1    12     11    92%
安全                     20   11    3    34     33    97%
Admin API                 9    4    0    13     12    92%
配置管理                  5    4    1    10     10   100%
可观测性                  6    4    0    10     10   100%
资源管理                  7    3    0    10     10   100%
消息持久化 (EventStore)   5    5    1    11     11   100%
测试策略                  5    4    2    11      5    45%
─────────────────────────────────────────────────────────────
总计                    100   62    9   170     155   91%
```
...
| 98 | **ADMIN-006** | GET /admin/health Gateway 健康检查 | 🔴 P0 | 🟢 PASS | `cmd/gateway/main.go` | 无需认证，DB 健康检查 |
| 99 | **ADMIN-007** | GET /admin/health/workers Worker 健康检查 | 🔴 P0 | 🟢 PASS | `cmd/gateway/main.go` | WorkerHealthStatuses() 真实探活 |
| 100 | **ADMIN-008** | GET /admin/logs 查询日志 | 🟡 P1 | 🟢 PASS | `cmd/gateway/main.go` | logRing ring buffer + Hub.LogHandler 接入 |
| 101 | **ADMIN-009** | POST /admin/config/validate 验证配置 | 🟡 P1 | 🟢 PASS | `cmd/gateway/main.go` | JSON body 解析，各字段结构验证 |
| 102 | **ADMIN-010** | GET /admin/debug/sessions/{id} 会话调试状态 | 🟡 P1 | 🟢 PASS | `cmd/gateway/main.go` | mutex_locked/turn_count/last_seq_sent/worker_health |
...
| 110 | **CONFIG-005** | 配置继承循环检测 | 🟡 P1 | 🟢 PASS | `config/config.go` | Inherits 字段 + loadRecursive 继承链 + ErrConfigCycle 循环检测 |
...
| 114 | **CONFIG-009** | 配置回滚 | ⚪ P2 | 🟢 PASS | `config/watcher.go` | History/Rollback/SetInitial + POST /admin/config/rollback |
...
| 130 | **RES-005** | 内存限制 RLIMIT_AS | 🔴 P0 | 🟢 PASS | `proc/manager.go:121-132` | syscall.Setrlimit RLIMIT_AS=512MB，非致命警告 |
...
| 133 | **RES-008** | per_user max_total_memory_mb 限制 | 🔴 P0 | 🟢 PASS | `session/pool.go` | PoolManager.AcquireMemory/ReleaseMemory，512MB/worker，默认 2GB/user |
| 134 | **RES-009** | Worker 可用性 99% 崩溃率控制 | 🔴 P0 | 🟢 PASS | `internal/metrics/metrics.go` | WorkerCrashesTotal counter + WorkerMemoryBytes gauge |
...
| 137 | **EVT-002** | Append-Only 触发器阻止 UPDATE 和 DELETE | 🔴 P0 | 🟢 PASS | `session/store.go` | 应用层 append-only：仅 Append 方法，INSERT OR IGNORE，无 Update/Delete |
| 138 | **EVT-003** | MessageStore 接口定义与编译时验证 | 🔴 P0 | 🟢 PASS | `session/store.go:289-309` | MessageStore 接口 4 方法 + SQLiteMessageStore 编译时验证 |
| 139 | **EVT-004** | Gateway 集成 EventStore 为可选插件 | 🔴 P0 | 🟢 PASS | `session/manager.go:36,79,87` | msgStore nil-safe 注入，Bridge.forwardEvents 按 Done 追加 |
| 140 | **EVT-005** | EventWriter 异步批量写入 | 🟡 P1 | 🟢 PASS | `session/store.go:360-380` | runWriter 单写 goroutine，1024 条或 500ms 批量 flush |
| 141 | **EVT-006** | Ownership 验证无循环依赖 | 🟡 P1 | 🟢 PASS | `session/store.go:307` | MessageStore.GetOwner 实现，无循环依赖 |
...
| 146 | **EVT-011** | EventStore 插件加载与配置解析 | 🟡 P1 | 🟢 PASS | `session/stores.go` | MessageStoreBuilder 注册表 + NewMessageStore 工厂 + init() 注册 sqlite |
...
| # | ID | 描述 | 优先级 | 状态 | 代码位置 | 备注 |
|---|----|------|--------|------|----------|------|
| 147 | **TEST-001** | 单元测试使用表驱动模式 | 🔴 P0 | 🟢 PASS | `*_test.go` | 10+ 测试文件使用表驱动；Worker adapter 无测试 |
| 148 | **TEST-002** | Mock 框架使用 testify/mock | 🔴 P0 | 🟢 PASS | `*_test.go` | testify/require 全覆盖；manager_test.go 使用 mockStore/mockWorker |
| 149 | **TEST-003** | Testcontainers 集成测试 | 🟡 P1 | ⚫ N/A | - | 无外部数据库依赖；单元测试已覆盖核心逻辑 |
| 150 | **TEST-004** | WebSocket Mock Server 用于集成测试 | 🟡 P1 | 🟡 IN_PROGRESS | `gateway/conn_test.go` | newTestWSServer httptest WebSocket mock 实现存在 |
| 151 | **TEST-005** | E2E 冒烟测试（Playwright） | 🔴 P0 | ⚫ N/A | - | 系统无 Web UI，无需 Playwright |
| 152 | **TEST-006** | 覆盖率目标 80%+ | 🔴 P0 | 🟡 IN_PROGRESS | `codecov.yml` | aep 86.9%/config 25%/gateway 14.9%/security 33.4%/session 56.8%；gateway 需 WS 集成测试；Worker adapter 均 0% |
| 153 | **TEST-007** | CI/CD 测试分层执行 | 🔴 P0 | 🟡 IN_PROGRESS | `codecov.yml` | codecov.yml 配置存在；GitHub Actions 未配置分层执行 |
...
| P0 | TEST-006 | 覆盖率目标 80%+ | 🟡 IN_PROGRESS（aep 86.9%/session 56.8%/gateway 14.9%） |
| P0 | TEST-007 | CI/CD 测试分层执行 | 🟡 IN_PROGRESS（codecov.yml 存在，GitHub Actions 未配置） |
...
| M9 EventStore 核心 + 测试基座 | 11 | 10 | 🟡 91% |
