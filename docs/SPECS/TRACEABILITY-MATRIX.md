...
> 文档版本: v1.0  |  最后更新: 2026-03-31  |  维护者: HotPlex Engineering
...
可观测性                  6    4    0    10      7    70%
资源管理                  7    3    0    10      8    80%
消息持久化 (EventStore)   5    5    1    11      7    64%
测试策略                  5    4    2    11      5    45%
─────────────────────────────────────────────────────────────
总计                     95   57    9   157     125   80%
```
...
| 111 | **CONFIG-006** | 配置热更新 fsnotify + 500ms 防抖 | 🟡 P1 | 🔴 TODO | - | 无 fsnotify watcher 和热更新机制 |
| 112 | **CONFIG-007** | 热更新动态字段与静态字段区分 | 🟡 P1 | 🔴 TODO | - | 无 HotReloadableFields/StaticFields 区分 |
| 113 | **CONFIG-008** | 配置变更审计日志 | 🟡 P1 | 🔴 TODO | - | 无配置变更审计 |
| 114 | **CONFIG-009** | 配置回滚 | ⚪ P2 | 🔴 TODO | - | 无版本历史和回滚机制 |
| 115 | **CONFIG-010** | 配置深度合并策略 | 🔴 P0 | 🟡 IN_PROGRESS | `config/config.go` | Viper Unmarshal 深度合并部分生效，但数组/切片完全替换 |
...
| 119 | **OBS-004** | RED 方法指标 API 层 | 🔴 P0 | 🟡 IN_PROGRESS | `metrics/metrics.go` | 指标声明存在，但 Inc()/Set() 调用大量缺失 |
| 120 | **OBS-005** | USE 方法指标基础设施层 | 🔴 P0 | 🟡 IN_PROGRESS | `metrics/metrics.go` | Pool/Gateway/Worker 指标声明存在，但未接入 |
| 121 | **OBS-006** | OTel Span 创建与上下文注入 | 🔴 P0 | 🔴 TODO | - | 无 go.opentelemetry.io/otel 导入 |
...
| # | ID | 描述 | 优先级 | 状态 | 代码位置 | 备注 |
|---|----|------|--------|------|----------|------|
| 136 | **EVT-001** | EventStore Schema 完整捕获所有事件类型 | 🔴 P0 | 🟡 IN_PROGRESS | `session/store.go` | events 表 schema 存在，但需验证完整性 |
...
| # | ID | 描述 | 优先级 | 状态 | 代码位置 | 备注 |
|---|----|------|--------|------|----------|------|
| 147 | **TEST-001** | 单元测试使用表驱动模式 | 🔴 P0 | 🔴 TODO | - | 无测试文件（security 包 0 测试文件）|
| 148 | **TEST-002** | Mock 框架使用 testify/mock | 🔴 P0 | 🔴 TODO | - | 无测试框架配置 |
| 149 | **TEST-003** | Testcontainers 集成测试 | 🟡 P1 | 🔴 TODO | - | 无 testcontainers 配置 |
| 150 | **TEST-004** | WebSocket Mock Server 用于集成测试 | 🟡 P1 | 🔴 TODO | - | 无 httptest WebSocket mock server |
| 151 | **TEST-005** | E2E 冒烟测试（Playwright） | 🔴 P0 | 🔴 TODO | - | 无 E2E 测试 |
| 152 | **TEST-006** | 覆盖率目标 80%+ | 🔴 P0 | 🔴 TODO | - | 无覆盖率报告 |
| 153 | **TEST-007** | CI/CD 测试分层执行 | 🔴 P0 | 🔴 TODO | - | 无 GitHub Actions CI 配置 |
...
### 🔴 P0 阻塞项（22 条，已完成 13 条）

| 优先级 | ID | 描述 | 影响 | 状态 |
|--------|----|------|------|------|
| P0 | EVT-001~006 | EventStore 核心（6 条） | 控制面持久化缺失 | ✅ 已完成 |
| P0 | WK-009 | bufio.Scanner 输出限制集成 | Worker 输出超限无检测 | ✅ 已完成 |
| P0 | RES-005 | RLIMIT_AS syscall.Setrlimit | 内存限制未生效 | ✅ 已完成 |
| P0 | OBS-004~005 | 指标 Inc/Set 调用缺失 | 可观测性失效 | ✅ 已完成 |
| P0 | OBS-006 | OTel Span 创建 | 无分布式追踪 | 🔴 TODO |
| P0 | TEST-001~003,005~007 | 测试基座（6 条） | 无法验证功能正确性 | ✅ 已完成 |
| P0 | CONFIG-006~008 | 配置热更新缺失 | 运行时无法调整参数 | 🔴 TODO |
...
| M8 可观测性 + 配置管理核心 | 13 | 7 | 🟡 54% |
| M9 EventStore 核心 + 测试基座 | 11 | 7 | 🟡 64% |
| M10 MVP 发布准备 (剩余 P0 + P1 收尾) | 152 | 67 | 🟡 44% |
