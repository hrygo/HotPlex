...
- **`chatapps/`**：**平台接入层**。将 HotPlex 连接到社交平台，支持多适配器架构：
  ...
  - **Telegram 适配器** (`chatapps/telegram/`): Bot API 集成
  - **DingTalk 适配器** (`chatapps/dingtalk/`): 企业消息与回调验证
  - **飞书适配器** (`chatapps/feishu/`):  Lark/飞书自定义机器人
  - **Discord 适配器** (`chatapps/discord/`): Discord 机器人集成
  - **WhatsApp 适配器** (`chatapps/whatsapp/`): WhatsApp Business API
  - 核心组件：
...
5.  **SDK 优先**：所有平台集成使用官方 SDK (slack-go, telegram-bot-api 等)，禁止手动实现协议。
...
| **Telegram** | Bot API | 内联键盘、命令 |
| **DingTalk** | 回调 + Webhook | 签名验证、企业安全 |
| **飞书** | 自定义机器人 | 卡片消息、交互回调 |
| **Discord** | Bot API | 富文本嵌入、斜杠命令 |
| **WhatsApp** | Business API | 模板消息、媒体 |
...
- [x] **多平台适配器**：Slack、Telegram、DingTalk、飞书、Discord、WhatsApp
...
| 特性 | Slack | Telegram | DingTalk | 飞书 | Discord | WhatsApp |
|------|-------|----------|----------|------|---------|----------|
...
| 斜杠命令 | ✅ | Commands | - | - | Slash | - |
| 签名验证 | ✅ | - | ✅ | ✅ | - | - |
| 交互按钮 | ✅ | Inline KB | ActionCard | Card | Buttons | CTA |
