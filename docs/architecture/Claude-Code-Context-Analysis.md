...
┌──────────────────────────────────────────────────────────────────────────┐
│ S0  ATTRIBUTION HEADER                cache: null (不缓存)              │
...
┌──────────────────────────────────────────────────────────────────────────┐
│ S2  STATIC CONTENT                    cache: global (跨用户/跨会话缓存)  │
│ ═════════════════════════════════════════════════════════════════════════ │
│  ┌─ # System ─────────────────────────────────────────────────────────┐ │
...
│   作用: 稳定的行为规范，变更极少，global 缓存命中率最高                  │
└──────────────────────────────────────────────────────────────────────────┘

    ┄┄┄┄ SYSTEM_PROMPT_DYNAMIC_BOUNDARY ┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄
    （全局缓存分界线 — 边界前 global 缓存，边界后不缓存）

┌──────────────────────────────────────────────────────────────────────────┐
│ S3  DYNAMIC CONTENT                   cache: null (每轮可变)            │
...
┌──────────────────────────────────────────────────────────────────────────┐
│ S4  SYSTEM CONTEXT APPEND             cache: null (不缓存)              │
...
④ splitSysPromptPrefix()                      api.ts:321
   → S0(null) + S1(null) + S2(global) + S3(null) + S4(null)
...
System Prompt:
  S0 Header   (null)      不缓存
  S1 Prefix   (null)      不缓存
  S2 Static   (global)    跨用户×会话×项目，Anthropic CDN 级别  (~15K tok, 节省 100%)
  ─── DYNAMIC BOUNDARY ───
  S3 Dynamic  (null)      每轮可变                              (~2K tok, 每次重发)
  S4 SysCtx   (null)      git status 会话级变化                 (~1K tok, 每次重发)

Messages:
  M0 User Ctx (随messages) CLAUDE.md + 日期，会话内稳定         (~5K tok, 节省 100%)
  M3+ History (增量缓存)    auto-compact 时 → cache miss + 重缓存

首轮: ~25K-45K tokens 全部 miss (占 200K window 的 12.5%-22.5%)
第二轮起: S2 global hit + M0 org hit → 节省 ~20K tokens/轮
```
...
`SYSTEM_PROMPT_DYNAMIC_BOUNDARY` 前 S2 对所有用户完全相同 → `global` 缓存。边界后任何运行时变量会产生 2^N 种组合，破坏全局缓存。

### 8.2 CLAUDE.md 的注入位置

CLAUDE.md 放在 `messages` 头部（M0），不在 `system` 参数中。优势：system 缓存不受用户指令影响；语义隔离（"may not be relevant"）；灵活更新。
...
```
  1. S2 静态硬编码规范        ← 不可变, global 缓存, ~15K tok
...
固定开销 (首轮, 不含历史):
  ...
  S0+S1          ~150 tok      不可控
  S2 Static    ~15,000 tok      不可控, global cache
