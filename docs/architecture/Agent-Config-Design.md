...
system[] (System Prompt — 5 段)
  S0  Attribution        (不可控)
  S1  CLI Prefix         (不可控, --append-system-prompt-file 自动切换为 SDK 模式)
  S2  Static Content     (不可控, ~15K tok)
  S3  Dynamic Content    (部分可控)
      ┃ ↓↓↓ B 通道注入点 (--append-system-prompt-file) ↓↓↓
...
| ------------------ | ------------------------------------- | --------------------------- |
| B 通道实现         | `--append-system-prompt-file` → S3 尾 | HTTP API `system` 字段 → S2 |
...
Step 3a: Claude Code Worker
  claude --append-system-prompt-file "/path/to/prompt.xml" \
...
├── B+C 统一注入 via --append-system-prompt-file (BuildSystemPrompt)
...
12. **临时文件注入 (全平台默认)** — 考虑到 `cmd.exe` 下内联 XML 标签被误认为 I/O 重定向，以及 POSIX 下大型 Prompt 可能超出 `ARG_MAX` 限制，Worker 适配器统一采用**临时文件注入**方案：先将 System Prompt 写入临时文件，再通过 `--append-system-prompt-file` 传递给底层进程。内置 200ms 文件锁重试机制确保清理成功。
