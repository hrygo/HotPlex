...
```bash
# 新建 session（Turn 1）
# ⚠️ 注意：无 --session-id flag，opencode 自动生成 sessionID
opencode run "Reply with exactly: OK" \
  --format json \
  --dir /path/to/project \
  -m provider/model

# 继续已有 session（Turn 2+，热复用）
opencode run "Next task" \
  --format json \
  --continue \
  --session <session-id> \
  --dir /path/to/project
...
```
Turn 1:  opencode run <prompt> --format json --dir /project
              ↓ (IsAlive() == true，进程常驻，opencode 自动生成 sessionID)
              ↓ Worker 从 step_start 事件的 sessionID 字段提取并存储
Turn 2+: opencode run <prompt> --continue --session <sessionID> --format json
              ↓ (同一 session 进程，pool 热复用)
Turn N:  opencode run <prompt> --continue --session <sessionID> --format json
```

> **⚠️ `--session-id` flag 不存在**：opencode 无新建 session 自定义 ID 的方式。Session ID 由 opencode 自动生成，Worker 必须从 `step_start` 事件的 `sessionID` 字段提取，并**落盘写入 SQLite `sessions` 表的 `worker_session_id` 字段**中。Gateway 对外（Client）仍维持主键 `id`（UUID），内部通信通过该映射解耦。

- **进程生命周期**：由 HotPlex `pool.go` 管理（`GetOrCreateSession` → `IsAlive()` → 复用）
- **Session 生命周期**：由 `opencode run --continue / --session` + opencode 内部 DB 管理
- **两层热复用**：
  1. **Pool 层**：同一 `opencode run` 进程在 Turn 间保持运行，无冷启动
  2. **Session 层**：`--continue --session <id>` 保持对话上下文连续
...
- **存储位置**：`~/.local/share/opencode/`（opencode 内部 SQLite DB）
- **Session 列表**：`opencode session list`
- **Session 删除**：`opencode session delete <id>`
- **与 Claude Code 对比**：机制完全相同，只是后端 DB 不同（opencode DB vs `.jsonl`）
...
func (p *OpenCodeServerWorker) BuildCLIArgs(sessionID string, opts *WorkerSessionOptions) []string {
    ...
    if opts.ResumeSession {
        // 使用数据库映射的 worker_session_id 进行恢复复用
        args = append(args, "--continue", "--session", opts.WorkerSessionID)
    } else {
        // 新建 session：opencode 无 --session-id flag
        // → Worker 从 step_start 提取 sessionID 后，回调通知 Gateway 更新 worker_session_id
    }
    return args
}
...
#### 8.2.2 OpenCode Server（`opencode serve`）
