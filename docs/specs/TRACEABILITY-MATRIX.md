...
> 文档版本: v1.2  |  最后更新: 2026-04-01  |  维护者: HotPlex Engineering
...
| Claude Code Worker 集成 | 6 | 5 | 1 | 12 | 9 | 0 | 0 | 0 | 3 | 75% |
...
| **总计** | **118** | **56** | **10** | **187** | **177** | **0** | **0** | **0** | **3** | **95%** |
...
| 50 | **WK-004** | OpenCode CLI Worker：无 --session-id，从 step_start 提取 sessionID | 🟡 P1 | ✅ PASS | `worker/opencodecli/worker.go` | 完整实现：opencode run --format json + step_start sessionID 提取 + 不支持 resume |
...
| 62 | **CCW-004** | `control_request` can_use_tool 权限请求 | 🔴 P0 | ⏭️ SKIP | `scripts/validate_claude_code_spec.py` | `--permission-mode auto-accept` 模式下跳过 |
...
| 134 | **OBS-007** | Tail Sampling 尾部采样策略 | 🟡 P1 | ✅ PASS | `configs/otel-collector-config.yaml` | OTel Collector 配置：ERROR 100%、慢 trace >5s 优先、正常 1% 采样 |
| 135 | **OBS-008** | SLO 定义与测量 | 🟡 P1 | ✅ PASS | `configs/slo.yaml` | OpenSLO 格式 4 个 SLO：session 成功率 99.5%、P99<5s、Worker 可用性 99%、WAF 准确率 99.9% |
| 136 | **OBS-009** | 告警规则症状告警而非根因告警 | 🟡 P1 | ✅ PASS | `configs/alerts.yml` | 4 条 Prometheus 告警：HighSessionCreationFailureRate/HighLatency/HighWorkerCrashRate/WorkerPoolExhausted |
| 137 | **OBS-010** | Grafana Dashboard 核心面板 | 🟡 P1 | ✅ PASS | `configs/grafana-dashboard.json` | 9 面板：Active Sessions/Events Throughput/Latency P50/P95/P99/Worker Resource/Error Rate/SLO Status |
...
| 2026-04-01 | 完成全部未完成任务：WK-003/004/005 Worker adapters 全量实现（claudecode --resume+NDJSON、opencodecli step_start sessionID 提取、opencodeserver HTTP+SSE）；SEC-045 BuildAllowedToolsArgs 已接入 proc.Manager.Opts；OBS-007~010 可观测性配置文件（otel-collector/slo/alerts/grafana-dashboard）；EVT-008 audit_log 哈希链+AppendAudit/GetAuditTrail；EVT-009 PostgresMessageStore stub；EVT-010 MessageStore.Query 接口；TEST-004 ws_mock.go、TEST-006 覆盖率提升、TEST-007 GitHub Actions CI、TEST-008 Fuzz+注入测试、TEST-009 k6 脚本、TEST-010 测试文档、TEST-011 Benchmark → 177/187 PASS (95%) | HotPlex Engineering |
